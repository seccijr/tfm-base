/* LA MACRO RANDOMSELECTlog REALIZA UN MÉTODO STEPWISE REPETIDAS VECES CON DIFERENTES ARCHIVOS TRAIN.

LA SALIDA INCLUYE UNA TABLA DE FRECUENCIAS DE LOS MODELOS QUE APARECEN SELECCIONADOS EN LOS DIFERENTES 
ARCHIVOS TRAIN

LOS MODELOS QUE SALEN MÁS VECES SON POSIBLES CANDIDATOS A PROBAR CON VALIDACIÓN CRUZADA
...
/*basura=F:\Técnicas de machine learning (Portela - Miércoles)\Tema 8\basura.txt*/ /*C:*/
...
%macro neuralbinariabasica(archivo=,listconti=,listclass=,vardep=,nodos=,corte=,semilla=,porcen=,algo=levmar);
	title '';
	data archivobase;
		set &archivo nobs=nume;
		ene=int(&porcen*nume);
		call symput('ene',left(ene));
	run;

	proc sort data=archivobase;
		by &vardep;
	run;
	...
	data salpredi;
		set salpredi;
		if p_&vardep.1>&corte/100 then predi1=1;
		else predi1=0;
	run;
	
	proc freq data=salpredi;
		tables predi1*&vardep /out=sal1;
	run;
